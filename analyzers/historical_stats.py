...
📊 СТАТИСТИКА ПО ОТДЕЛЬНЫМ ТОКЕНАМ:
        ...
        # Add strategy guidance based on actual win rate
        if stats['win_rate_percent'] < 20:
            strategy = f"📉 Текущий винрейт низкий ({stats['win_rate_percent']}%). Рекомендуется строгий отбор сигналов."
        elif stats['win_rate_percent'] < 40:
            strategy = f"📊 Текущий винрейт средний ({stats['win_rate_percent']}%). Отбирай сигналы с чёткими паттернами."
        else:
            strategy = f"📈 Текущий винрейт хороший ({stats['win_rate_percent']}%). Можно брать качественные сетапы."
...
- Стоп-лосс: -5% от цены входа
- {strategy}
- Ищи: цена падает (price_change_6h/24h < 0), но покупки превышают продажи = разворот
- Избегай токенов с price_change_24h > +20% (перегреты)
- Избегай токенов с винрейтом < 40% в статистике выше
- Отдавай предпочтение токенам с винрейтом > 50%
- Лучше 0 сигналов, чем слабые сигналы
        """
